...
name: self-distill
description: |
  自我蒸馏 Skill：从用户自己的钉钉数据（消息+文档+听记+多维表格）中提炼自我画像，输出
  「自我数字人 + 趣味报告（MBTI/SBTI/动物）+ 工作报告 + 野兽派×二次元风个人名片 PNG」。
  ...
  触发词：「自查」「蒸馏自己」「看看我」「看看我自己」「我是谁」「自我画像」「我的野兽派名片」
cli_version: ">=1.0.15"
...
> 「AI 不知道你是谁，但你自己留下的痕迹知道。」

这个 Skill 把"自我认知"从一次性 prompt 变成可复用的工程化产物——但**你的聊天记录、文档正文只活在内存里，闭着眼睛跑、关掉就没了**。

## 触发条件

| 用户说 | 模式 |
|--------|------|
| `自查` / `蒸馏自己` / `看看我` / `看看我自己` | 完整跑一遍（含知情同意） |
| `我是谁` / `自我画像` | 完整跑一遍 |
