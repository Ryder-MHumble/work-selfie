...
name: worksona
description: |
  Worksona：从用户授权的钉钉工作痕迹（消息+文档+听记+多维表格）中提炼职场人格，输出
  ...
  触发词：「Worksona」「自查」「蒸馏自己」「看看我」「看看我自己」「我是谁」「自我画像」「我的野兽派名片」
cli_version: ">=1.0.15"
...
> 「你不是在上班，你是在扮演一种很具体的职场角色。」

这个 Skill 把工作聊天、文档、听记和表格里留下的信号，变成一份可解释的自我分析报告和一张能分享的职场人格卡——但**你的聊天记录、文档正文只活在内存里，闭着眼睛跑、关掉就没了**。

## 触发条件

| 用户说 | 模式 |
|--------|------|
| `Worksona` / `自查` / `蒸馏自己` / `看看我` / `看看我自己` | 完整跑一遍（含知情同意） |
| `我是谁` / `自我画像` | 完整跑一遍 |
